...
export default async function SubjectPage({ params }: Props) {
  ...
  if (!GATE_SUBJECTS.includes(subject as any)) notFound()
    ...
    <div className="space-y-6 animate-fade-in">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm mt-0.5" style={{ color: 'var(--text-secondary)' }}>
                {totalCount} subtopics · {completedCount} completed · ~{totalCount * 45} min total
              </p>
            </div>
          </div>
      ...
      <div className="space-y-5">
        {Object.keys(byTopic).length === 0 ? (
          <div className="card p-12 text-center border-dashed" style={{ borderColor: 'var(--border-subtle)', background: 'transparent' }}>
            <div className="w-16 h-16 rounded-full mx-auto flex items-center justify-center text-2xl mb-4"
              style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-subtle)' }}>
              📭
            </div>
            <h3 className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>No topics yet</h3>
            <p className="text-sm mt-1 max-w-sm mx-auto" style={{ color: 'var(--text-secondary)' }}>
              Use the form above to add the first subtopic for {subject}.
            </p>
          </div>
        ) : (
          Object.entries(byTopic).map(([topicName, subtopicsRaw]) => {
            const subtopics = subtopicsRaw || []
            const groupCompleted = subtopics.filter(s => progressMap.get(s.id)?.completed).length
            const groupPct = subtopics.length > 0 ? Math.round((groupCompleted / subtopics.length) * 100) : 0
