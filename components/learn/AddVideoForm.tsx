...
export function AddVideoForm({ topicId, onAdded }: Props) {
  ...
  const getYtId = (u: string) => u.match(/(?:v=|youtu\.be\/)([^&\n?#]+)/)?.[1]

  const submit = async () => {
    if (!url.trim() || !title.trim()) { setError('Title and URL are required'); return }
    if (!getYtId(url)) { setError('Please enter a valid YouTube URL'); return }
    ...
    else setError(data.error ?? 'Failed to add video')
    setSaving(false)
  }
    ...
    <div className="card p-5 space-y-3 border-2" style={{ borderColor: 'rgba(99,102,241,0.3)' }}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 font-semibold text-sm" style={{ color: 'var(--text-primary)' }}>
          <PlayCircle className="w-4 h-4 text-red-500" /> Add YouTube Video
        </div>
        <button onClick={() => setOpen(false)} className="btn-ghost p-1"><X className="w-4 h-4" /></button>
      </div>
      <input className="input text-sm" placeholder="Video title (e.g. Banker's Algorithm Explained)"
        value={title} onChange={e => setTitle(e.target.value)} />
      <input className="input text-sm" placeholder="YouTube URL (e.g. https://youtube.com/watch?v=...)"
